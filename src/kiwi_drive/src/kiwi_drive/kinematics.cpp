...
#include <kiwi_drive/FrameToFrame.h>
#include <kiwi_drive/KinematicsForward.h>
#include <kiwi_drive/KinematicsInverse.h>
...
bool mobileToWorld(kiwi_drive::FrameToFrame::Request &request, kiwi_drive::FrameToFrame::Response &response) {
...
bool worldToMobile(kiwi_drive::FrameToFrame::Request &request, kiwi_drive::FrameToFrame::Response &response) {
    worldToMobileCore(request.input.x, request.input.y, response.output.x, response.output.y);
}

bool forwardMobile(kiwi_drive::KinematicsForward::Request &request, kiwi_drive::KinematicsForward::Response &response) {
...
bool forwardWorld(kiwi_drive::KinematicsForward::Request &request, kiwi_drive::KinematicsForward::Response &response) {
    forwardMobile(request, response);
    mobileToWorldCore(response.output.x, response.output.y, response.output.x, response.output.y);
    return true;
}

bool inverseMobile(kiwi_drive::KinematicsInverse::Request &request, kiwi_drive::KinematicsInverse::Response &response) {
...
bool inverseWorld(kiwi_drive::KinematicsInverse::Request &request, kiwi_drive::KinematicsInverse::Response &response) {
...
int main(int argc, char **argv){
    ...
    ros::NodeHandle node;
    {
        ...
        KDL::Chain chain;
        if (!tree.getChain("base_link", "wheel_back_link", chain)) {
            ROS_ERROR("Failed to get chain from KDL tree.");
            return -1;
        }
        ROS_INFO("---------------------------------- Got chain from KDL tree -----------------------------");
    ...
    ros::Subscriber subscriber = node.subscribe("pose/world", 10, onPoseWorldMessage);
    ros::spin();
    return 0;
}
